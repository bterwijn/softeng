...
def main():
    ...
    parser.add_argument("-g", type=str, help="the game to play, default 'ttt'",
                        choices=["ttt", "c4", "chess"], default="ttt")
    parser.add_argument("-s", type=str, help="the size of the game",
                        default="")
    parser.add_argument("-p1", type=str,
                        help="""type of player1: 'h' (human, default), 'r' (random), 'a1' (AI 1 move), 'a2' (AI 2 moves), 'aa' (AI all moves), 'dN' (AI N moves), 'dNh' (AI N moves with heuristic) """, default="h")
    parser.add_argument("-p2", type=str,
                        help="""type of player2: 'h' (human), 'r' (random, default), 'a1' (AI 1 move), 'a2' (AI 2 moves), 'aa' (AI all moves), 'dN' (AI N moves), 'dNh' (AI N moves with heuristic) """, default="r")
...
def game_factory(game, size, players):
    if game == 'ttt':
        ...
        if size:
            rows, cols = [int(i) for i in size.split('x')]
            size = (rows, cols)
        else:
            size = (6, 7)
        return Connect_Four(size, players)
    elif game == 'chess':
        return Chess_Adapter(players)
    else:
